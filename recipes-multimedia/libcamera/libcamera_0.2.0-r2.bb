SUMMARY = "Linux libcamera framework"
SECTION = "libs"

LICENSE = "GPL-2.0-or-later & LGPL-2.1-or-later"

LIC_FILES_CHKSUM = "\
    file://LICENSES/GPL-2.0-or-later.txt;md5=fed54355545ffd980b814dab4a3b312c \
    file://LICENSES/LGPL-2.1-or-later.txt;md5=2a4f4fd2128ea2f65047ee63fbca9f68 \
"

SRC_URI = " \
        git://git.libcamera.org/libcamera/libcamera.git;protocol=https;branch=master \
        file://0001-libcamera-x-linux-isp-v5.0.0.patch \
"

# tag v0.2.0
SRCREV = "89227a428a82e724548399d35c98ea89566f9045"

PE = "1"

S = "${WORKDIR}/git"

DEPENDS = "python3-pyyaml-native python3-jinja2-native python3-ply-native python3-jinja2-native udev gnutls chrpath-native libevent libyaml evision-lib linux-stm32mp"
DEPENDS += "${@bb.utils.contains('DISTRO_FEATURES', 'qt', 'qtbase qtbase-native', '', d)}"

PACKAGES =+ "${PN}-gst"

PACKAGECONFIG ??= "gst"
PACKAGECONFIG[gst] = "-Dgstreamer=enabled,-Dgstreamer=disabled,gstreamer1.0 gstreamer1.0-plugins-base"

LIBCAMERA_PIPELINES ??= "dcmipp"

EXTRA_OEMESON = " \
    -Dpipelines=${LIBCAMERA_PIPELINES} \
    -Dv4l2=true \
    -Dcam=enabled \
    -Dlc-compliance=disabled \
    -Dtest=false \
    -Ddocumentation=disabled \
    -Devision_algo=true \
"

RDEPENDS:${PN} = "${@bb.utils.contains('DISTRO_FEATURES', 'wayland qt', 'qtwayland', '', d)}"

inherit meson pkgconfig python3native module-base

do_configure:prepend() {
    sed -i -e 's|py_compile=True,||' ${S}/utils/ipc/mojo/public/tools/mojom/mojom/generate/template_expander.py
    cp ${RECIPE_SYSROOT}/usr/include/evision/*.h ${S}/src/ipa/dcmipp/algorithms
}

do_install:append() {
    chrpath -d ${D}${libdir}/libcamera.so
    chrpath -d ${D}${libexecdir}/libcamera/v4l2-compat.so
}

do_package:append() {
    bb.build.exec_func("do_package_recalculate_ipa_signatures", d)
}

do_package_recalculate_ipa_signatures() {
    local modules
    for module in $(find ${PKGD}/usr/lib/libcamera -name "*.so.sign"); do
        module="${module%.sign}"
        if [ -f "${module}" ] ; then
            modules="${modules} ${module}"
        fi
    done

    ${S}/src/ipa/ipa-sign-install.sh ${B}/src/ipa-priv-key.pem "${modules}"
}

python populate_packages:prepend () {
    # get the recipe version of the stm32 dcmipp kernel module
    import os
    import glob

    pkgdata_dir = d.getVar('PKGDATA_DIR')
    file_pattern = os.path.join(pkgdata_dir, 'runtime', 'kernel-module-stm32-dcmipp*')
    files = glob.glob(file_pattern)

    if not files:
        bb.fatal("No files found matching pattern: {}".format(file_pattern))

    file_path = files[0]  # Assuming you want the first match if there are multiple

    pkgv = None
    pr = None
    with open(file_path, 'r') as f:
        for line in f:
            if line.startswith('PKGV:'):
                pkgv = line.split(':')[1].strip()
            elif line.startswith('PR:'):
                pr = line.split(':')[1].strip()

    # update the RDEPENDS with the stm32 dcmipp kernel module version
    pn = d.getVar('PN')
    kernel_version = d.getVar('KERNEL_VERSION')
    rdepends = d.getVar('RDEPENDS:' + pn)
    rdepends = rdepends + " kernel-module-stm32-dcmipp-%s (>= %s-%s) " % (kernel_version, pkgv, pr)
    d.setVar('RDEPENDS:' + pn, rdepends)
}

FILES:${PN} += " ${libexecdir}/libcamera/v4l2-compat.so"
FILES:${PN}-gst = "${libdir}/gstreamer-1.0"

RDEPENDS:${PN} += " \
    evision-lib \
"

RDEPENDS:${PN}-gst += " \
    gstreamer1.0-python \
    libcamera (>= ${EXTENDPKGV}) \
"

# libcamera-v4l2 explicitly sets _FILE_OFFSET_BITS=32 to get access to
# both 32 and 64 bit file APIs.
GLIBC_64BIT_TIME_FLAGS = ""
